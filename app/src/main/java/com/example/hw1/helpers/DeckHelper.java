...
public class DeckHelper {
    ...
    public static ArrayList<Card> createDeck(Context context, char[] LETTERS) {
        ...
        ((Base_Activity) context).game_BAR_progress.setMax(deck.size() / 2);
        return deck;
    }
    ...
    public static void drawCards(Context context, ArrayList<Card> deck) {
        ...
        deck.remove(0);
        UIHelper.convertIMG(context,first_card.getId(), ((Base_Activity) context).game_IMG_player_card1);
        UIHelper.convertIMG(context,second_card.getId(), ((Base_Activity) context).game_IMG_player_card2);
        if (first_card.getValue() > second_card.getValue()) {
            player1_score++;
            ((Base_Activity) context).game_LBL_score1.setText("" + player1_score);
        } else if (first_card.getValue() < second_card.getValue()) {
            player2_score++;
            ((Base_Activity) context).game_LBL_score2.setText("" + player2_score);
        }
    }
